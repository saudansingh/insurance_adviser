...
# 🛠️ FIXED: Added init_db alongside the rest of your model imports
from database import async_session, get_or_create_user, load_memory, SessionSummary, init_db

# Load environment variables
load_dotenv()

# 🛠️ FIXED: Changed log level to INFO so connection states & errors show up in GCP Cloud Logging
        ...
        async with async_session() as session:
            if summary_id:
                ...
                if existing:
                    ...
                    logger.info(f"Successfully updated summary row {summary_id} for user {user_id}")
                    return summary_id
            ...
            logger.info(f"Successfully created summary row {new_summary.id} for user {user_id}")
            return new_summary.id
    except Exception as e:
        logger.error(f"Failed to save session summary to database: {e}")
        return summary_id or 0


# 🛠️ FIXED: Added database engine initialization pool to the server lifespan startup
@asynccontextmanager
async def lifespan(app: FastAPI):
    """Validate configuration and initialize database on application startup."""
    if not GEMINI_API_KEY:
        logger.warning("WARNING: GEMINI_API_KEY is not set. WebSocket endpoints will not work.")
    
    try:
        logger.info("Connecting and initializing database pool...")
        await init_db()
        logger.info("Database connection pool initialized successfully.")
    except Exception as db_init_err:
        logger.error(f"CRITICAL: Failed to run init_db() on startup: {db_init_err}")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async with async_session() as db_session:
        logger.info(f"New WebSocket session established for user: {user_email}")
        ...
        async def handle_websocket_messages():
            try:
                ...
                logger.info("User dropped WebSocket stream connection.")
            except Exception as e:
                ...
                async for event in gemini_client.start_session(
                    ...
                    if event and isinstance(event, dict):
                        ...
                        if text_content:
                            ...
                            # Periodic sentence background save
        ...
        try:
            ...
            if user_id and current_call_text_segments:
                ...
                if conversation_text:
                    try:
                        logger.info(f"Connection ending. Executing SHIELDED final database write for user: {user_id}")
                        await asyncio.shield(save_session_summary(session_summary_id, user_id, conversation_text))
                    except Exception as save_err:
                        logger.error(f"Failed to save context block on final drop: {save_err}")
